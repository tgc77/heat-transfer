...
void salvarDadosEmArquivo(EqHeat mesh)
{
     ...
     if ((arq = fopen("MeshPoints_ImplicitMethod.txt", "w")) == NULL){
           ShowMessage("Erro, o arquivo não pode ser aberto!");
           fclose(arq);
           Abort();
     }
     ...
     fprintf(arq, "==========================================================================" \
                  "==============================================\n");
     fprintf(arq, "      Distribuição de temperatura nos pontos (Xi) da malha calculados " \
                  "pelo metodo Implícito por diferenças finitas.\n");
     fprintf(arq, "===========================================================================" \
                  "=============================================\n");
     ...
     fprintf(arq, "u(t, xi) , com t variando de 0 até %d, e xi com i = 0, 1,..., %d são " \
           "os nós da malha...\n\n", mesh->N-1, mesh->I-1);
     for(n = 0; n < mesh->N; n++) {
          for(i = 0; i < mesh->I; i++) {
...
REAL *method_Gauss_Jordan(REAL **SEL, const int LIN, const int COL)
{
     ...
     for(lin = 0; lin < LIN; lin++) {
          ...
          if(!mSysIndeterminado) {
              ...
              ShowMessage("Sistema de equações lineares indeterminado.");
              delete []U;
              Abort();
          }
     }
